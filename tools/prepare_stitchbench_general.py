...
def main() -> int:
    parser = argparse.ArgumentParser(description="Prepare StitchBench General metadata and GES-GSP graph files.")
    ...
    for path in (graphs_root, logs_root, experiment_root / "0_results", experiment_root / "1_debugs"):
    ...
    with manifest_csv.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(
            handle,
            fieldnames=["dataset", "category", "image_count", "image_files", "data_dir", "graph_file"],
        )
        writer.writeheader()
        for row in rows:
            out = row.copy()
